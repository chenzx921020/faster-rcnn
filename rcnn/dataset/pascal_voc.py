...
class PascalVOC(IMDB):
    def __init__(self, root_path, devkit_path,data_gt):
        """
        ...
        :return: imdb object
        """
        #super(PascalVOC, self).__init__( root_path, devkit_path, data_gt)  # set self.name
        ...
        self.name = 'dms'
        self.num_images = len(open(root_path+'/'+data_gt,'r').readlines())
    ...
    def image_path_from_index(self, img_name):
        ...
        """
        image_file = self.root_path+'/'+ img_name
        #print image_file
        assert os.path.exists(image_file), 'Path does not exist: {}'.format(image_file)
        return image_file

    def gt_roidb(self,data_gt):
        """
        ...
        if os.path.exists(cache_file):
            ...
            return roidb
        lines = open(self.root_path+'/'+data_gt,'rb').readlines()

        gt_roidb = [(self.load_pascal_annotation(index,lines)) for index in range(0,len(lines))]
    ...
    def load_pascal_annotation(self, index ,lines):
        """
        ...
        roi_rec['image'] = self.image_path_from_index(tmp[0])
